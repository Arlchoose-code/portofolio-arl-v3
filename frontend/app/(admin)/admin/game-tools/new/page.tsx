...
export default function NewGameToolPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await gameToolsApi.create(formData);
      if (res.status) {
        toast.success('Game baru berhasil ditambahkan');
        router.push('/admin/game-tools');
      } else {
        toast.error(res.message || 'Gagal menambahkan game');
      }
    } catch {
  ...
  return (
    <FormWrapper
      title="Tambah Game Checker Baru"
      backHref="/admin/game-tools"
      onSubmit={handleSubmit}
      isLoading={loading}
      submitLabel="Tambah Game"
    >
