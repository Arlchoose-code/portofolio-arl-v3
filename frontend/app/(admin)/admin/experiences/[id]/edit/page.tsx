...
export default function EditExperiencePage() {
  ...
  useEffect(() => {
    if (id) {
      experiencesApi.get(id).then((res) => {
        if (res.status && res.data) {
          ...
          setWorkMode(e.work_mode || 'remote');
          setStartDate(e.start_date || '');
          setEndDate(e.end_date || '');
          setIsCurrent(Boolean(e.is_current));
          setDescription(e.description || '');
          setSortOrder(e.sort_order || 0);

          let stack: string[] = [];
          if (Array.isArray(e.tech_stack)) stack = e.tech_stack;
          else if (typeof e.tech_stack === 'string') {
            try {
              stack = JSON.parse(e.tech_stack);
            } catch {
              stack = e.tech_stack ? e.tech_stack.split(',') : [];
            }
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await experiencesApi.update(id, {
        company,
        position,
        type,
        location,
  ...
  return (
    <FormWrapper
      title={`Edit Pengalaman: ${position} at ${company}`}
