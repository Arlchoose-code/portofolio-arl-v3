...
import { TwoFactorTool } from '@/components/tools/TwoFactorTool';
...
export function TwoFactorClient({ initialSetting }: Props) {
  const isEnabled = initialSetting ? initialSetting.is_enabled !== false : true;
  const toolName = initialSetting?.name || '2FA Authenticator & TOTP Generator';
  const toolSlug = initialSetting?.slug || '2fa-generator';
  ...
  if (!isEnabled) {
    return (
      <div className="pt-28 pb-20 max-w-6xl mx-auto px-6">
        ...
        <ToolDisabledState
          toolName={toolName}
          message="Layanan 2FA Authenticator sedang dinonaktifkan sementara oleh administrator."
        />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <TwoFactorTool toolSetting={initialSetting} />
    </motion.div>
  );
}
