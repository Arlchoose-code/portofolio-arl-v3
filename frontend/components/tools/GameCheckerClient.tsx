...
export function GameCheckerClient({ initialSetting }: Props) {
  const isEnabled = initialSetting ? initialSetting.is_enabled !== false : true;
  const toolName = initialSetting?.name || 'Cek Nickname Game Online';
  const toolSlug = initialSetting?.slug || 'game-checker';
  ...
  return (
    ...
    >
      ...
      <GameCheckerTool toolSetting={initialSetting} isStandalonePage={true} />
    </motion.div>
  );
}

export default GameCheckerClient;
