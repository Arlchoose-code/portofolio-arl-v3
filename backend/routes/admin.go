...
func RegisterAdminRoutes(r *gin.RouterGroup, cfg *config.Config) {
	...
	adminProtected.Use(middlewares.AuthMiddleware(cfg), middlewares.RequireAdminRole())
	{
		...
		adminProtected.POST("/mailbox/senders", mailboxCtrl.AddSender)
		adminProtected.DELETE("/mailbox/senders/:id", mailboxCtrl.DeleteSender)
		adminProtected.PUT("/mailbox/senders/default", mailboxCtrl.SetDefaultSender)
